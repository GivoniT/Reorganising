...
def run_engine(engine_name: str, factory) -> pd.DataFrame:
    ...
    # Also compute pre-game prediction, confidence, and correctness for this engine
    pred_correct_flags = []     # 1 if correct, -1 if incorrect
    pred_sign_flags = []        # +1 if engine predicted WIN_TEAM, -1 if predicted LOSE_TEAM
    pred_conf_flags = []        # confidence for the predicted side (max(p, 1-p))
    ...
    for _, row in results_df.iterrows():
        ...
        # Engine's predicted side and confidence
        if p_win >= 0.5:
            pred_sign_flags.append(1)                  # predicts WIN_TEAM
            pred_conf_flags.append(p_win)              # confidence is p_win
            pred_correct_flags.append(1)               # correct prediction
        else:
            pred_sign_flags.append(-1)                 # predicts LOSE_TEAM
            pred_conf_flags.append(1.0 - p_win)        # confidence is 1 - p_win
            pred_correct_flags.append(-1)              # incorrect prediction

        # Now update the engine with the actual result
        engine.record_game(win, lose, gdate)

    # Attach prediction columns to results_df for this engine
    results_df[f"PRED_{engine_name}"] = pred_sign_flags           # +1/-1 prediction label
    results_df[f"PRED_CONF_{engine_name}"] = pred_conf_flags      # confidence in [0.5, 1.0]
    results_df[f"PRED_CORRECT_{engine_name}"] = pred_correct_flags
    results_df[f"PRED_SCORE_{engine_name}"] = [ps * pc for ps, pc in zip(pred_sign_flags, pred_conf_flags)]
...
print("[PATCH] Score columns + updated summaries are active.")
...
# %% Accuracy & confidence-weighted score summary for each engine
pred_cols = [c for c in results_df.columns if c.startswith("PRED_CORRECT_")]
score_cols = [c for c in results_df.columns if c.startswith("PRED_SCORE_")]
if pred_cols:
    print("\n=== Model Accuracies ===")
    acc_rows = []
    total_games = len(results_df)
    for c in pred_cols:
        engine_name = c.replace("PRED_CORRECT_", "")
        # For ±1 labels: accuracy = (# of +1) / N
        correct = int((results_df[c] == 1).sum())
        acc = float(correct) / total_games if total_games else 0.0
        acc_rows.append({"engine": engine_name, "correct": correct, "total": total_games, "accuracy": acc})
        print(f"{engine_name:10s}: {acc:.2%}  (" + f"{correct}/{total_games})")
    pd.DataFrame(acc_rows).to_csv("engine_accuracy_summary.csv", index=False)
    print("✅ engine_accuracy_summary.csv written")
else:
    print("No prediction columns found; skip accuracy summary.")

if score_cols:
    print("\n=== Model Confidence-Weighted Scores ===")
    score_rows = []
    for c in score_cols:
        engine_name = c.replace("PRED_SCORE_", "")
        score_sum = float(results_df[c].sum())
        score_mean = float(results_df[c].mean()) if len(results_df) else 0.0
        score_rows.append({"engine": engine_name, "score_sum": score_sum, "score_mean": score_mean})
        print(f"{engine_name:10s}: sum={score_sum:.3f}  mean={score_mean:.5f}")
    pd.DataFrame(score_rows).to_csv("engine_score_summary.csv", index=False)
    print("✅ engine_score_summary.csv written")
else:
    print("No score columns found; skip score summary.")
